...
def search_logs(when):
    ...
    sql = "SELECT v.* from view_SystemEvents_compact v LEFT JOIN Exclude e ON (v.Message LIKE concat('%',e.exclude,'%')) WHERE e.exclude IS NULL AND v.DeviceReportedTime BETWEEN '{0} 00:00:00' AND '{0} 23:59:59'".format(
        when)
...
def sign_logs(time, user, mess, now):
    ...
    '''
    sql = "INSERT INTO signed(date,sign,mess,signdate) VALUES ('{0}','{1}','{2}','{3}')".format(
        time, user, mess, now)
    mysql_query(sql)
...
def add_exclude(ex_this, status):
    ...
    '''
    sql = "INSERT INTO Exclude(exclude,user,adddate,status) VALUES('{0}','{1}','{2}','{3}')".format(
        ex_this, os.environ["REMOTE_USER"], date.today(), status)
    mysql_query(sql)
...
def add_alert(ad_this, status, host):
    ...
    Exclude this from logs
    '''
    sql = "INSERT INTO alert(alert,user,adddate,status,hosts) VALUES('{0}','{1}','{2}','{3}','{4}')".format(
        ad_this.strip(), os.environ["REMOTE_USER"], date.today(),
        status.strip(), host.strip())
    mysql_query(sql)
